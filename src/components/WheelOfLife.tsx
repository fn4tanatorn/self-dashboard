  ...
  return (
    <div className="flex flex-col gap-6 lg:flex-row">
      <div className="flex-1">
        ...
        <div className="flex flex-col gap-3">
          {WHEEL_CATEGORIES.map((cat) => (
            <div key={cat} className="flex items-center gap-3">
              <span className="w-28 shrink-0 text-sm capitalize text-neutral-600">{cat}</span>
              <input
                type="range"
                min={1}
                max={10}
                ...
                className="flex-1"
              />
              <span className="w-5 text-right text-sm font-medium text-neutral-800">
                {scores[cat]}
              </span>
            </div>
          ))}
        ...
        <button
          onClick={save}
          className="mt-4 rounded-lg bg-neutral-900 px-3 py-2 text-sm font-medium text-white hover:bg-neutral-700"
        >
