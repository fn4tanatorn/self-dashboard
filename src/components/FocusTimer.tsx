  ...
  return (
    <div className="flex flex-col items-center gap-6">
        ...
        {MODES.map((m) => (
          <button
            ...
            className={`rounded-md px-3 py-1.5 text-xs font-medium disabled:opacity-40 ${
              mode === m.key ? "bg-neutral-900 text-white" : "text-neutral-500 hover:bg-neutral-100"
            }`}
          >
            {m.label}
          </button>
      ...
      <div className="text-center">
        <p className="text-6xl font-semibold tabular-nums tracking-tight text-neutral-900">
          ...
          return (
            <div className="flex items-center gap-1.5">
              {Array.from({ length: POMODORO_CYCLES_BEFORE_LONG_BREAK }, (_, i) => (
                <span
                  key={i}
                  className={`h-2 w-2 rounded-full ${i < filled ? "bg-neutral-900" : "bg-neutral-200"}`}
                />
              ))}
            </div>
          );
      ...
      <div className="flex items-center gap-2">
        {phase === "idle" && (
          <button
            onClick={onStart}
            className="flex items-center gap-2 rounded-lg bg-neutral-900 px-5 py-2.5 text-sm font-medium text-white hover:bg-neutral-700"
          >
            <Play size={16} /> Start
          </button>
        )}
        {phase === "focus" && (
          <>
            <button
              ...
              className="flex items-center gap-2 rounded-lg border border-neutral-200 px-4 py-2.5 text-sm font-medium text-neutral-700 hover:bg-neutral-50"
            >
