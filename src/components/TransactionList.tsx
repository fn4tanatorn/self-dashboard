    ...
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <input
          ...
          placeholder="Note"
          className="min-w-[120px] flex-1 rounded-lg border border-neutral-200 bg-white px-3 py-2 text-sm placeholder:text-neutral-400 focus:border-neutral-400"
        />
        <input
          value={date}
      ...
      <div className="flex flex-col divide-y divide-neutral-100">
        ...
        {sorted.map((t) => (
          <div key={t.id} className="group flex items-center gap-3 py-2.5">
            <span className="w-24 shrink-0 text-xs text-neutral-400">{t.date}</span>
            <span className="rounded-full bg-neutral-100 px-2 py-0.5 text-[11px] font-medium text-neutral-500">
              {t.category}
            </span>
            <span className="flex-1 truncate text-sm text-neutral-600">{t.note}</span>
            <span
              className={`text-sm font-medium ${t.type === "income" ? "text-emerald-600" : "text-neutral-800"}`}
            >
              {t.type === "income" ? "+" : "-"}
